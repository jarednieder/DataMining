...
public class PartitionMatrix {
	...
	public static class Map extends MapReduceBase implements
		...
		// All lines in the input file have the following form:
		// doc_000.sum-hash-3 87
		// WARNING: If you change the output format of the GenerateSignature
		// MapReduce
		// you have to change this regular expression.
		Pattern pattern = Pattern
				.compile("([a-z0-9_\\.]+)\\s+(\\d+)\\s+(\\d+)");
		...
		public void map(LongWritable key, Text value,
			...
			if (m.matches()) {
				...
				output.collect(new Text(filename), new Text(hashFunction + " "
						+ minHash));
			}
		}
	}
	...
	public static class Reduce extends MapReduceBase implements
		...
		// Here each hash function has an array of coefficients A and an offset
		// B, each of which is smaller than MAX_COEF.
		VectorHashFunction[] hashFunctions = HashFamily
				.generateVectorHashFamily(MAX_COEF, NROWS, NBANDS, SEED);

		// Reducer calculates the hash for each column of each band, given
		// signature matrix column as input.
		public void reduce(Text key, Iterator<Text> values,
			...
			while (values.hasNext()) {
				...
				if (m.matches()) {
					...
					throw new IOException("Line doesn't match the pattern!");
			}

			// TODO: Calculate and output the hash value of each vector for each
			// band.
			long[] band;
