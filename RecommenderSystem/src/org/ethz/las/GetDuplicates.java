...
public class GetDuplicates {
	...
	public static class Map extends MapReduceBase implements
		...
		// Used to read in the file line by line. It says that each line is in
		// the form
		// one or more digits followed by one or more spaces followed by one or
		// more digits
		// folowed by one or more spaces folowed by more than one symbol which
		// can be either lowercase
		// letter, digit or a dot.
		Pattern p = Pattern.compile("(\\d+)\\s+(\\d+)\\s+([a-z0-9_\\.]+)");

		/**
		 * Mapper reads the input and forwards it to the output setting (hash
		 * value, band) -> (document).
		 */
		public void map(LongWritable key, Text value,
			...
			if (m.matches()) {
				int hashValue = Integer.parseInt(m.group(1));
		...
		public void reduce(Text key, Iterator<Text> values,
			...
			for (int i = 0; i < candidates.length; i++) {
				for (int j = i + 1; j < candidates.length; j++) {
					output.collect(new Text(candidates[i]), new Text(
							candidates[j]));
				}
			}
		}
